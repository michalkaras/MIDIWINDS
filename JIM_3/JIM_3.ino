...
void UpdateDisplay(){
  ...
  static const char *instrument[] = { "SAX-AV", "FLUTE", "OBOE", "E-HORN", "CORNET",\
                                      "SAX-T", "FLUTE-N", "PAN-FL", "CLARIN", "ATMOS", "F-HORN","CH-OOH", "BRASS",\
...
double averageEXP2() {
  ...
  const float sensorvalueEXP2 = map((a.acceleration.x * 1000), -6000, 0, 0, 127);
//  Serial.print("sensorvalueEXP2: ");
//  Serial.println(sensorvalueEXP2);                
  RET_AVG(48, constrain(sensorvalueEXP2, 0, 127));  
}
...
double averagePB() {
  ...
  RET_AVG(24, constrain(sensorvaluePB, -6, 6));                    
}

void SendNoteInfo() {
  const float EXP1 = constrain(averageEXP1(), 24, 124);
  const float VOL1 = constrain(averageVOL1(), 24, 124);
  const float EXP2 = constrain(averageEXP2(), 24, 127);
  const float VOL2 = constrain(averageVOL2(), 24, 124);
  const float PB = constrain(averagePB(), -6, 6);
