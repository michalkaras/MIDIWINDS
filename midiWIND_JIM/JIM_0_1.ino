...
PRESS DOWN - note
PRESS UP - halfnote
PRESS RIGHT & LEFT - asigned function
...
void setup(void) {
  ...
  BLEMidiServer.begin("JIM ver.: 0_1");
...
// Works with midiDesigner app only
void ReadAUX2Pin() {
...
double averageEXP2() {
  ...
  const float sensorvalueEXP2 = map((a.acceleration.x * 1000), -5700, 200, 0, 127);            
...
double averagePB() {
  sensors_event_t a, g, temp;
  mpu.getEvent(&a, &g, &temp);
  const float sensorvaluePB = map((g.gyro.z * 1000), 500, 0, 6, 0);               
  RET_AVG(36, constrain(sensorvaluePB, -6, 6));                    
}

double averageMOD() {
  sensors_event_t a, g, temp;
  mpu.getEvent(&a, &g, &temp);
  const float sensorvalueMOD = map((a.acceleration.y * 1000), -400, -1500, 0, 16);              
  RET_AVG(48, constrain(sensorvalueMOD, 0, 127));                    
}
...
void SendNoteInfo() {
  ...
  const float PB = constrain(averagePB(), -64, 64);
  ...
  BLEMidiServer.pitchBend(1, PB, 127);
