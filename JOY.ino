...
*/
  ...
  /*{
    char *msgs[] = {(char *)"KARAS"};
    oledFill(&ssoled, 0, 1);
    oledWriteString(&ssoled, 0,10,0,msgs[rc], FONT_STRETCHED, 0, 1);
    delay(500);
  // Display "version:" at (x=0, y=30)
    oledWriteString(&ssoled, -3, 10, 3, (char *)"SAX_2", FONT_SMALL, 0, 1);
...
//  pinMode(BRASS_PIN, INPUT_PULLUP);         // pin 40
//  pinMode(STRINGS_PIN, INPUT_PULLUP);       // pin 39
...
void UpdateDisplay() {
  ...
  if (old_scale != scale_index) {
    // Clear the area where the scale is displayed  
//    oledFill(&ssoled, 0, 1); // Clear the display (or use a specific color)
...
double averageEXP1() {
  ...
  RET_AVG(128, map(outputValueEXP1, 0, 127, 32, 127));  
}
double averageVOL1() {
  const float sensorValue = analogRead(BREATH_PIN);
  const float outputValueVOL2 =  map((sensorValue - offset) * sin(0.85), 0, 170, 0, 127);
  Serial.println(outputValueVOL2); 
  RET_AVG(128, map(outputValueVOL2, 0, 127, 12, 127));                   
}
double averageEXP2() {
  const float sensorValue = analogRead(BREATH_PIN);
  const float outputValueEXP2 = map((sensorValue - offset) * sin(0.55), 0, 200, 0, 127);
  Serial.println(outputValueEXP2);                    
  RET_AVG(128, map(outputValueEXP2, 0, 127, 32, 127));  
}
double averageVOL2() {
  const float sensorValue = analogRead(BREATH_PIN);
  const float outputValueVOL2 = map((sensorValue - offset) * sin(0.90), 0, 180, 0, 127);
  Serial.println(outputValueVOL2);  
  RET_AVG(64, map(outputValueVOL2, 0, 127, 12, 127));                   
}
/*
void SendAccelerometerInfo() {
...
void SendBreathInfo() {
  ...
  const float VOL1 = constrain(averageVOL1(), 0, 124);
  Serial.println (averageVOL1());
  const float EXP2 = constrain(averageEXP2(), 0, 124);
  Serial.println(averageEXP2());
  const float VOL2 = constrain(averageVOL2(), 0, 124);
  Serial.println (averageVOL2());
 
  if (mouthTouched) { 
    BLEMidiServer.controlChange(0, 11, EXP1); // Expression for channel 1  
    BLEMidiServer.controlChange(0, 7, VOL1);  // Volume for channel 1      
  }
  if (aux1Pressed) {
